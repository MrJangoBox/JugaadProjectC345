...
*/
void Strategy::executeAttack(Player *player, vector<Player>* listCopy)
{
}
...
*/
void AggresiveAttack::executeAttack(Player *player, vector<Player>* listCopy){
	...
	for(int i = 0; i < playerCountries->size(); i++) {
		...
		for(int j = 0; j < countryNeighbours.size(); j++)
		{
			...
			{
				if(player->getCountriesOwned()->at(i) == countryNeighbours.at(j))
					invalidChoice = true;
			}
			...
			if(playerCountry->getArmiesPlaced() != 0 && countryNeighbours.at(j)->getArmiesPlaced() < playerCountry->getArmiesPlaced() 
				&& countryNeighbours.at(j)->getArmiesPlaced() >= 0 && invalidChoice == false)
			{
...
*/
void DefensiveAttack::executeAttack(Player *player, vector<Player>* listCopy){
...
*/
void RandomAttack::executeAttack(Player *player, vector<Player>* listCopy){
	vector<Country*>* playerCountries = player->getCountriesOwned();
	...
	for(int i = 0; i < playerCountries->size(); i++) {
		...
		if(attackCond == 1)
		{
			...
			for(int j = 0; j < countryNeighbours.size(); j++)
			{
				...
				if(playerCountry->getArmiesPlaced() != 0 && attackNeighbour == 1 && countryNeighbours.at(j)->getArmiesPlaced() >= 0 && invalidChoice == false)
				{
					Country *dfndrCountryP = countryNeighbours.at(j);
					cout << "\n\n********************* Another Batlle ********************\n";
					cout << "\nBattle between " << atkrCountryP->getCountryName() << " and " << dfndrCountryP->getCountryName() << endl;
