...
vector<Country*>* StartupPhase:: getLoadedMap() {
	return LoadedMap;
}
...
//Function that starts the game from loaded file.
void StartupPhase:: StartGame(vector<Player>* lPlayers) {
	vector<Player>* loadedPlayers = lPlayers;
	MainPlayPhase mainPhaseObj = MainPlayPhase(loadedPlayers, listOfCPtr.size(), LoadedMap);
}
