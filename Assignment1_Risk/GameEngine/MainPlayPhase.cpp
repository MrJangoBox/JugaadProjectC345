...
MainPlayPhase:: MainPlayPhase(vector <Player> *playersPtr, int NoOfCountries) {
	listCopy = playersPtr;
	numberOfCountries = NoOfCountries; // Total number of countries in the map.
}
...
void MainPlayPhase::Reinforce(Player *player) {
	...
	//While the player still has Armies, place them in countries if the useer chooses to.
...
void MainPlayPhase::Fortify(Player *player) {
	...
	while ( (toupper(ch) != 'Y') && (toupper(ch) != 'N')) 
		if(toupper(ch) == 'Y') {
			...
			cout << "Which country do you want to move armies from? Select the option. " << endl;
			from = SelectCountry(player);

			cout << "Which country do you want to move it to? Select the option. " << endl;
